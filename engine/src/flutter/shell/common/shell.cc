...
// |Animator::Delegate|
void Shell::OnAnimatorBeginFrameForDisplay(fml::TimePoint frame_target_time,
                                           uint64_t frame_number,
                                           int64_t display_id,
...
// |Animator::Delegate|
void Shell::OnAnimatorDrawLastLayerTreesForDisplay(
    std::unique_ptr<FrameTimingsRecorder> frame_timings_recorder,
    const std::set<int64_t>& view_ids) {
  FML_DCHECK(is_set_up_);
  ...
  auto task = fml::MakeCopyable(
      [rasterizer = rasterizer_->GetWeakPtr(),
       ...
       view_ids]() mutable {
        if (rasterizer) {
          rasterizer->DrawLastLayerTreesForViews(
              std::move(frame_timings_recorder), view_ids);
        }
      });
  task_runners_.GetRasterTaskRunner()->PostTask(task);
}
...
// |Animator::Delegate|
void Shell::OnAnimatorEmptyFrameForDisplay(int64_t display_id,
                                           const std::set<int64_t>& view_ids) {
  FML_DCHECK(is_set_up_);
  FML_DCHECK(task_runners_.GetUITaskRunner()->RunsTasksOnCurrentThread());
...
// |Animator::Delegate|
bool Shell::OnAnimatorFrameOpportunityOutcome(FrameOpportunityId opportunity_id,
                                              int64_t display_id,
                                              int64_t target_id,
  ...
  if (settings_.on_frame_opportunity_outcome) {
    return settings_.on_frame_opportunity_outcome(opportunity_id, display_id,
                                                  target_id, outcome);
  }
  return false;
}
...
// |Rasterizer::Delegate|
bool Shell::OnFrameOpportunityOutcome(FrameOpportunityId opportunity_id,
                                      int64_t display_id,
                                      int64_t target_id,
                                      FrameOpportunityOutcome outcome) {
  if (settings_.on_frame_opportunity_outcome) {
...
// |Rasterizer::Delegate|
void Shell::OnFrameOpportunityBackpressured(
    int64_t display_id,
