...
class Shell final : public PlatformView::Delegate,
 ...
 private:
  ...
  // |Animator::Delegate|
  bool OnAnimatorFrameOpportunityOutcome(
      FrameOpportunityId opportunity_id,
      int64_t display_id,
      int64_t target_id,
      FrameOpportunityOutcome outcome) override;

  // |Rasterizer::Delegate|
  bool OnFrameOpportunityOutcome(FrameOpportunityId opportunity_id,
                                 int64_t display_id,
                                 int64_t target_id,
                                 FrameOpportunityOutcome outcome) override;

  // |Rasterizer::Delegate|
  void OnFrameOpportunityBackpressured(
      int64_t display_id,
      const std::set<int64_t>& target_ids) override;
