...
MakeRenderTargetFromBackingStoreImpeller(
  ...
  if (!vulkan->image_view) {
    FML_LOG(ERROR) << "Embedder supplied null Vulkan image view. "
                   << "image_view is required for Impeller.";
    return nullptr;
  }
  ...
  if (!format.has_value()) {
    FML_LOG(ERROR) << "Unsupported Vulkan pixel format: "
...
InferExternalViewEmbedderFromArgs(const FlutterCompositor* compositor,
  ...
  if (!c_present_render_target_callback) {
    FML_LOG(ERROR) << "present_render_target_callback must be provided.";
    return {nullptr, true};
  }
