...
/// Render relevance for one view. This is deliberately separate from Dart
/// application lifecycle and input focus: an obscured view remains alive but
/// produces no new raster demand until made visible again.
typedef enum {
  kFlutterAvioViewVisibilityVisible = 0,
  kFlutterAvioViewVisibilityObscured = 1,
  kFlutterAvioViewVisibilitySuspended = 2,
} FlutterAvioViewVisibility;

typedef struct {
  /// The size of this struct. Must include `visibility`.
  size_t struct_size;

  /// The view whose render relevance changed.
...
//------------------------------------------------------------------------------
/// @brief      Sets render relevance for one view.
///
